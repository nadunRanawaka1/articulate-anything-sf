...
def move_all_joints(cfg: DictConfig, scene, cameras, robot, joints_dict, video_writers, same_speed: bool = True):
    """
    Move all manipulatable joints simultaneously and render videos from all camera views.
    ...
    Args:
        ...
        robot: Robot object
        joints_dict: List of (joint_name, joint_type, limits) tuples
        video_writers: Dict of video writers per camera
        
    Returns:
        List of joint trajectories
    """
    num_joints = len(joints_dict)
    
    if num_joints == 0:
        logging.warning("No manipulatable joints found")
        return []
    
    # Determine movement direction and range for each joint
    joint_configs = []
    for joint_idx, (joint_name, joint_type, (lower_limit, upper_limit)) in enumerate(joints_dict):
        # Auto-determine direction: if lower < 0, move from upper to lower, else lower to upper
        if cfg.simulation_params.joint_move_dir == "auto":
            if lower_limit < 0:
                start, end = upper_limit, lower_limit
            else:
                start, end = lower_limit, upper_limit
        elif cfg.simulation_params.joint_move_dir == "move_up":
            start, end = lower_limit, upper_limit
        else:  # move_down
            start, end = upper_limit, lower_limit
        
        # Assign different speeds and phases to each joint for clear visual distinction
        # Use varying speeds and staggered starts so each joint's motion is easily identifiable
        if same_speed:
            speed = 1.0
        else:
            speed_multipliers = [1.0, 0.6, 0.8, 0.4, 0.9, 0.5, 0.7]  # Varied speeds
            speed = speed_multipliers[joint_idx % len(speed_multipliers)]
        ...
        if cfg.get('verbose', False):
            logging.info(f"Joint {joint_idx} ({joint_name}): speed={speed:.2f}")
    
    # Animate all joints with different speeds
    trajectory = []
    for step in range(cfg.simulation_params.num_steps):
        qpos = robot.get_qpos()
        
        # Update all joint positions with individual speeds
        for joint_idx, config in enumerate(joint_configs):
            # Calculate progress with speed multiplier
            raw_progress = step / cfg.simulation_params.num_steps
            adjusted_progress = raw_progress * config['speed']
            # Clamp to [0, 1]
            progress = min(1.0, adjusted_progress)
            
            qpos[joint_idx] = config['start'] + progress * (config['end'] - config['start'])
    ...
    if cfg.flip_video:
        for camera_name, video_writer in video_writers.items():
            video_writer.release()
            video_path = os.path.join(
                cfg.output.dir, f"video_all_joints_{camera_name}.mp4")
            flip_video(video_path)
    
    logging.info(f"Created videos for {num_joints} joints from {len(cameras)} camera views")
    return trajectory
...
def capture_video(cfg: DictConfig, scene, robot, cameras: dict, manipulatable_joints):
    # Special case: move all joints simultaneously
    if cfg.simulation_params.joint_name == "all_simultaneous":
        initial_qpos = robot.get_qpos()
        robot.set_qpos(initial_qpos)
        scene.step()
        scene.update_render()
        
        with VideoWriterManager(cfg, "all_joints") as video_writers:
            move_all_joints(cfg, scene, cameras, robot, manipulatable_joints, video_writers)
        return
